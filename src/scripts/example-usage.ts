...
async function loadPokemonList(): Promise<NamedAPIResource[]> {
  try {
    ...
    console.log('First 10 Pokémon:', pokemonList.slice(0, 10).map((p: NamedAPIResource) => p.name));
...
async function loadPokemonById(id: number): Promise<Pokemon | null> {
  try {
    ...
    const pokemonFile = files.find(file => file.startsWith(`${String(id)}-`) && file.endsWith('.json'));
    
    if (!pokemonFile) {
      console.error(`Pokemon #${String(id)} not found. Run "npm run fetch-pokemon:pokemon" first.`);
      return null;
    }
    ...
    console.log(`Types: ${pokemon.types.map((t: PokemonType) => t.type.name).join(', ')}`);
  ...
  try {
    ...
    for (const pokemon of pokemonList.slice(0, 50)) {
      try {
        ...
        const pokemonDir = path.join(DATA_DIR, 'api', 'v2', 'pokemon');
        const files = await fs.readdir(pokemonDir);
        const pokemonFile = files.find(file => file.startsWith(`${String(pokemonId)}-`) && file.endsWith('.json'));
        ...
        if (pokemonFile) {
          ...
          const hasType = pokemonData.types.some((t: PokemonType) => t.type.name === typeName);
          if (hasType) {
    ...
    matchingPokemon.forEach(pokemon => {
      console.log(`#${String(pokemon.id)} ${pokemon.name} (${pokemon.types.join('/')})`);
    });
...
async function loadPokemonImages(id: number): Promise<PokemonImages | null> {
  try {
    ...
    const imagesDir = path.join(DATA_DIR, 'images', 'pokemon', `${String(id)}-${pokemonName}`);
